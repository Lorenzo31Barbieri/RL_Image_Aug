...
# Enhanced state dimension (logits + confidence measures + correctness + step info)
STATE_DIM = NUM_CLASSES + 5  # 10 logits + 5 additional features
...
def evaluate_agent_detailed(agent, classifier_model, eval_episodes=200):
    """
    Detailed evaluation of agent performance.
    """
...
def train_rl_agent():
    """
    Main training function with improved strategies.
    """
    ...
    prefill_episodes = min(1000, num_total_episodes // 10)
    ...
    for episode in range(num_total_episodes):
        ...
        while not done and steps < max_steps_per_episode:
            ...
            if len(agent.replay_buffer) > batch_size and episode > warmup_episodes:
                for _ in range(2):  # Multiple learning steps per environment step
        ...
        if global_episode_counter % eval_freq == 0 and global_episode_counter > warmup_episodes:
            ...
            if eval_reward > best_eval_reward:
                ...
                if not os.path.exists('./models'):
                    os.makedirs('./models')
                torch.save(agent.q_network.state_dict(), './models/best_improved_dqn_model.pth')
        ...
        if global_episode_counter % 2000 == 0:
            if not os.path.exists('./models'):
                os.makedirs('./models')
            torch.save(agent.q_network.state_dict(), 
                      f'./models/improved_dqn_episode_{global_episode_counter}.pth')
    ...
    if not os.path.exists('./models'):
        os.makedirs('./models')
    torch.save(agent.q_network.state_dict(), './models/final_improved_dqn_model.pth')
...
def create_comprehensive_plots(episode_rewards, loss_history, evaluation_history, agent):
    ...
    if evaluation_history:
        ...
        stats_text = f"""Final Training Results:
...
Positive Episodes: {(np.array(episode_rewards) > 0).mean():.1%}
    ...
    plt.tight_layout()
    plt.savefig('./plots/improved_comprehensive_training_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print("Training plots saved to './plots/improved_comprehensive_training_analysis.png'")
